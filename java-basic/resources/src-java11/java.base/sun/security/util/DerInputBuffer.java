...
class DerInputBuffer extends ByteArrayInputStream implements Cloneable {
    ...
    static int checkPaddedBits(int numOfPadBits, byte[] data,
    ...
    public byte[] getBitString(int len) throws IOException {
        ...
        checkPaddedBits(numOfPadBits, buf, pos + 1 , (pos + len), allowBER);
        // minus the first byte which indicates the number of padding bits
        byte[] retval = Arrays.copyOfRange(buf, pos + 1, (pos + len));
        if (allowBER && numOfPadBits != 0) {
    ...
    BitArray getUnalignedBitString() throws IOException {
        ...
        int validBits = checkPaddedBits(unusedBits,
            buf, pos + 1 /*start*/ , (pos + len) /*end*/, allowBER);

        return new BitArray(validBits, buf, pos + 1);
    }
