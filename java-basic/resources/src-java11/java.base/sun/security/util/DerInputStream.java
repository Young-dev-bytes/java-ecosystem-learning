...
public class DerInputStream {
    ...
    public BitArray getUnalignedBitString() throws IOException {
        ...
        if ((length != 0) && (buffer.available() < length)) {
            throw new IOException("Length exceeds available capacity: " + length);
        }
        ...
        int validBits = DerInputBuffer.checkPaddedBits(excessBits,
            repn, 0 /*start*/, repn.length /*end*/, buffer.allowBER);
    ...
    protected DerValue[] readVector(int startLen) throws IOException {
        ...
        if (buffer.available() < len) {
            throw new IOException("Length exceeds available capacity: " + len);
        }
