...
def build_single_chatbot(chatbot_id):
    ...
    with gr.Group(elem_id="share-region-named" + chatbot_id):
        with gr.Row(elem_id="model_selector_row" + chatbot_id):
            ...
            # fresh_btn.click(build_model_selector, [], model_selector)
            btn_state = gr.Button("模型未加载", scale=1, interactive=False)
            # btn_state123 = gr.Button("模型加载中", scale=1, interactive=False)
            button_state = gr.State("load")
            a=True
        ...
        with gr.Accordion("管理模型", open=True, visible=True):
            with gr.Row(elem_id="model_selector_row" + chatbot_id):
                ...
                # async def load_model_and_listen(model_name, btn_state):
                #     print(btn_state)
                #     load_response = await load_model_btn_operate(model_name)
                #     btn_state_update, button_state_update, *rest = load_response
                #     await asyncio.sleep(1)  # 非阻塞的等待
                ...
                async def load_model_and_listen(model_name):
                    load_response = await load_model_btn_operate(model_name)
                    btn_state_update, button_state_update, *rest = load_response
                    ...
                    while True:
                        ...
                        if resp_status_load == "Running":
                            ...
                            # 每次循环更新按钮状态
                            yield [
                                gr.update(value=f"模型加载中...{count}s", interactive=False),
                                "loading",
                                gr.update(interactive=False),
                                gr.update(interactive=False),
                                gr.update(interactive=False),
                ...
                async def load_model_btn_operate(model_name):
                    resp_load_model = load_model(model_name)
                    btn_state = gr.update("模型加载中--", scale=1, interactive=False)
                ...
                #     show_progress="hidden",
                # ).then(
                #     listen_load_model_btn_status,
                #     inputs=[model_selector],
                #     outputs=[btn_state, button_state, model_selector, load_btn, unload_btn],
                #     show_progress="hidden",
                # )
                load_btn.click(
                    load_model_and_listen,
                    inputs=[model_selector],
                    outputs=[btn_state, button_state, model_selector, load_btn, unload_btn],
                    show_progress="hidden",
                )
